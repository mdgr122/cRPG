...
// lambda function that takes in an equip slot and returns a wstring for that slot.
auto slot2str = [&](Stats::EquipSlots slot)
{
    switch (slot)
    {
        ...
        case Stats::EquipSlots::ACCESSORY:
            return L"ACCESSORY: ";
        case Stats::EquipSlots::NO_SLOT: assert(false && "Should always have a slot!");
            return L"NO_SLOT: ";
        default: assert(false && "Should always have a slot!");
            return L"";
    }
};
...
void StatusMenuState::DrawPlayerInfo()
{
    ...
    const auto &xp_str    = L"EXP: " + xp + L" / " + xp_to_next;
    ...
    int i = 0;
    // unordered map of equip slots to a shared pointer of equipment
    ...
    //unordered map of wstrings to int value
    ...
    for (const auto &[stat, value] : stat_list)
    {
        ...
        // Get modifier at for the specific stat we're looping through
        m_Console.Write(STAT_LABEL_X_POS, STAT_LABEL_START_Y_POS + i, stat);
        m_Console.Write(STAT_VAL_X_POS, STAT_LABEL_START_Y_POS + i, std::to_wstring(value));
        // because this is a sharedptr, it could be a nullptr, so we need to check that by checking if it's equipped
        i++;
    }
}
