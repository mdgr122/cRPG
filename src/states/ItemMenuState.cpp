...
ItemState::ItemState(Player &player, Console &console, StateMachine &stateMachine, Keyboard &keyboard)
    ...
    , m_Keyboard(keyboard)
    , m_Player(player)
    , m_MenuSelector{console, keyboard, {L"Items", L"Key Items"},
                     //SelectorParams{40, 10, 2, 25, 0}
                     SelectorParams{40, 10, 2, 0, 0} // x, y, cols, current x, current y
    }
    , m_ItemSelector{console, keyboard, std::bind(&ItemState::OnItemSelect, this, _1, _2), std::bind(&ItemState::RenderItem, this, _1, _2, _3),
                     std::vector<std::shared_ptr<Item>>(), SelectorParams{30, 14, 2, 0, 0}
...
void ItemState::OnMenuSelect(int index, std::vector<std::wstring> data)
{
    ...
    // selector for either the item or key item, enters into that ItemChoice
}
...
void ItemState::RenderItem(int x, int y, std::shared_ptr<Item> item)
{
    ...
    if (index < data.size())
    {
        ...
        // Getting center of screen and subtracting half of the size of the item_desc to center it in the screen.
    }
}
...
void ItemState::FocusOnMenu()
{
    ...
    // Hides Item Selector bc we don't want to see it when not in that area
