...
GameMenuState::GameMenuState( Party&        party, Console&         console,
                              StateMachine& stateMachine, Keyboard& keyboard )
	: m_Party(party), m_Console(console), m_StateMachine(stateMachine),
	  m_Keyboard(keyboard), m_MenuSelector{
		  console, keyboard,
		  {
			  L"Shop", L"Items", L"Magic", L"Equipment", L"Stats", L"Order",
			  L"Save", L"Exit"
		  },
	  ...
	  , m_PlayerSelector{
		  ...
		  // _1, _2, std::bind placeholder args.
		  std::bind(&GameMenuState::OnDrawPlayerSelect, this, _1, _2, _3),
		  // _1, _2, std::bind placeholder args.
		  party.GetParty(),
		  //SelectorParams{69, 13, 1, 0, 10} 
		  SelectorParams{ 69, 13, 1, 0, 0, 10, 10 }
		  // x=69, y=13, columns=1, current_x=0, current_y=0, spacing_x=20, spacing_y=10
	  ...
	  m_FirstChoice{ -1 }    // -1 means no choice
	  , m_SecondChoice{ -1 } // -1 means no choice
	  , m_eSelectType{ SelectType::NONE }
{
	m_MenuSelector.SetSelectionFunc(
		std::bind(&GameMenuState::OnMenuSelect, this, _1, _2));
}
...
{
	if (m_bInMenuSelect) m_PlayerSelector.HideCursor();
...
void GameMenuState::ProcessInputs()
{
	if (m_bInMenuSelect)
	{
		...
		// Only want to process the menu selector if we are infact in the menu selector
	}
	else
	{
		if (m_Keyboard.IsKeyPressed(KEY_BACKSPACE))
		{
			...
			m_eSelectType   = SelectType::NONE;
			m_FirstChoice   = m_SecondChoice = -1; // resets playpos to -1 
		...
		// Only want to select player selector if we're in player selection
	}
}
...
void GameMenuState::DrawPanels()
{
	...
	m_Console.Write(menu_x_pos,
	                2,
	                L"  ____         _ ___                 ",
	                GREEN);
	m_Console.Write(menu_x_pos,
	                3,
	                L" |_  /___ __ _| | __|__ _ _ __ _ ___ ",
	                GREEN);
	m_Console.Write(menu_x_pos,
	                4,
	                L"  / // -_) _` | | _/ _ \\ '_/ _` / -_)",
	...
	m_Console.Write(menu_x_pos,
	                5,
	                L" /___\\___\\__,_|_|_|\\___/_| \\__, \\___|",
	                GREEN);
	m_Console.Write(menu_x_pos,
	                6,
	                L"                           |___/     ",
	...
	// SMALL UNDER SAVE
	m_Console.DrawPanelHorz(m_PanelBarX - 1,
	                        (m_ScreenHeight - 2),
	                        PANEL_BARS + 2,
	                        BLUE); // BOTTOM LAST
...
void GameMenuState::DrawPlayerInfo()
{
	...
	for (const auto& player : m_Party.GetParty())
	{
		...
		std::wstring hp_string    = L"HP: " + hp + L" / " + hp_max;
		std::wstring level_string = L"Lvl: " + level + L" Exp: " + xp + L" / " +
			xp_to_next;
...
{
	const auto& party = m_Party.GetParty();
	switch (index)
	{
		case 0: m_eSelectType = SelectType::SHOP;
			m_StateMachine.PushState(std::make_unique<ShopState>(
				m_Party,
				m_Console,
				m_StateMachine,
				m_Keyboard,
				"Assets/xml_files/WeaponShopDefs_1.xml"));
			// ShopState(Party& party, Console& console, StateMachine& stateMachine, Keyboard& keyboard, const std::string & shopFilepath);
			break;
		case 1: m_eSelectType = SelectType::ITEM;
			break;
		case 2: m_eSelectType = SelectType::MAGIC;
			break;
		case 3: m_eSelectType = SelectType::EQUIPMENT;
			break;
		case 4: m_eSelectType = SelectType::STATS;
			break;
		case 5: m_eSelectType = SelectType::ORDER;
			break;
		case 6:
			// TODO: Save Game
			return;
		case 7: m_bExitGame = true;
			return;
		default: return;
	}
...
void GameMenuState::OnPlayerSelect( int                                  index,
                                    std::vector<std::shared_ptr<Player>> data )
{
	...
	switch (m_eSelectType)
	{
		...
		case SelectType::ITEM:
			m_StateMachine.PushState(
				std::make_unique<ItemState>(*player,
				                            m_Console,
				                            m_StateMachine,
				                            m_Keyboard));
			break;
		...
		case SelectType::EQUIPMENT:
			m_StateMachine.PushState(
				std::make_unique<EquipmentMenuState>(
					*player,
					m_Console,
					m_StateMachine,
					m_Keyboard));
		...
		case SelectType::STATS:
			m_StateMachine.PushState(
				std::make_unique<StatusMenuState>(
					*player,
					m_Console,
					m_StateMachine,
					m_Keyboard));
...
void GameMenuState::OnDrawPlayerSelect( int                     x, int y,
...
{
	if (m_FirstChoice < 0 && m_SecondChoice < 0) return;
