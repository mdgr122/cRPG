...
GameMenuState::GameMenuState(Party &party, Console &console, StateMachine &stateMachine, Keyboard &keyboard)
    ...
    , m_Keyboard(keyboard)
    , m_MenuSelector{console, keyboard, {L"Shop", L"Items", L"Magic", L"Equipment", L"Stats", L"Order", L"Save", L"Exit"},
                     //SelectorParams{30, 8, 1, 0, 4}
                     SelectorParams{30, 8, 1, 0, 0}} // x, y, columns, currentX, currentY, spacingX, spacingY
    , m_PlayerSelector{console, keyboard, std::bind(&GameMenuState::OnPlayerSelect, this, _1, _2),
...
void GameMenuState::OnMenuSelect(int index, std::vector<std::wstring> data)
{
    ...
    {
        case 0:
            m_eSelectType = SelectType::SHOP;
            m_StateMachine.PushState(std::make_unique<ShopState>(m_Party, m_Console, m_StateMachine, m_Keyboard, "Assets/xml_files/WeaponShopDefs_1.xml"));
