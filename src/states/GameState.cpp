...
GameState::GameState( Console&      console, Keyboard& keyboard,
...
//, m_TestInventory{} // TO BE REMOVED
{
	...
	auto potion = ItemCreator::CreateItem(Item::ItemType::HEALTH,
	                                      L"Potion",
	                                      L"Heals for a small amount",
	                                      25,
	                                      50);
	auto strong_potion = ItemCreator::CreateItem(
		Item::ItemType::HEALTH,
		L"Strong Potion",
		L"Heals a large amount of health",
		250,
		300);
	...
	auto Player1 = std::make_shared<Player>(L"Player 1",
	                                        L"1",
	                                        m_Party->GetInventory(),
	                                        1,
	                                        100,
	                                        20);
	auto Player2 = std::make_shared<Player>(L"Player 2",
	                                        L"2",
	                                        m_Party->GetInventory(),
	                                        1,
	                                        100,
	                                        20);
	auto Player3 = std::make_shared<Player>(L"Player 3",
	                                        L"3",
	                                        m_Party->GetInventory(),
	                                        1,
	                                        100,
	...
	auto       shopParameters = shop_loader.CreateShopParametersFromFile(
		"Assets/xml_files/ArmourShopDefs_1.xml");
...
void GameState::OnEnter()
{
	...
	EquipmentLoader el{ "Assets/xml_files/WeaponDefs.xml" };
...
void GameState::ProcessInputs()
{
	...
	if (m_Keyboard.IsKeyPressed(KEY_ENTER))
	{
		m_StateMachine.PushState(
			std::make_unique<GameMenuState>(*m_Party,
			                                m_Console,
			                                m_StateMachine,
			                                m_Keyboard));
		return;
