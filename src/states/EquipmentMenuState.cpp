...
EquipmentMenuState::EquipmentMenuState(Player &player, Console &console, StateMachine &stateMachine, Keyboard &keyboard)
    ...
    , m_Keyboard{keyboard}
    , m_Player{player}
    , m_MenuSelector{console, keyboard, {L"EQUIP", L"REMOVE", L"OPTIMIZE"},
                     //SelectorParams{42, 10, 3, 15, 2}
                     SelectorParams{38, 10, 3, 0, 0}
                     // X, Y, COLS, CURR_X, CURR_Y [X of text, Y of text, col of text, x of selector, y of selector]
    }
    , m_EquipSlotSelector{console, keyboard, std::bind(&EquipmentMenuState::OnSlotSelect, this, _1, _2), std::bind(&EquipmentMenuState::RenderEquipSlots, this, _1, _2, _3),
                          // this func
    ...
    , m_EquipmentSelector{console, keyboard, std::bind(&EquipmentMenuState::OnEquipSelect, this, _1, _2), std::bind(&EquipmentMenuState::RenderEquip, this, _1, _2, _3),
...

{
    ...
    // Selectors are shown by default, so we want to hide the EquipmentSelector explicitly
    m_EquipSlotSelector.HideCursor();
    // Selectors are shown by default, so we want to hide the EquipSlotSelector explicitly
}
...
void EquipmentMenuState::ProcessInputs()
{
    ...
    {
        if (m_Keyboard.IsKeyPressed(KEY_BACKSPACE))
        // equipment slot selector - working
        {
...
void EquipmentMenuState::DrawPlayerInfo()
{
    ...
    for (const auto &[stat, value] : stats_list)
    {
        ...
        // The GetModifier() func takes in a string which is a key to the actual value
        m_Console.Write(STAT_LABEL_X_POS, STAT_LABEL_START_Y_POS + i, stat);
        // Writing the stat to the buffer
...
void EquipmentMenuState::OnSlotSelect(int index, std::vector<std::wstring> data)
{
    ...
    // the selector that shows up once selected weapons/headgear/armour/footwear/accessory
    m_bInSlotSelect = false;
}
...
void EquipmentMenuState::RenderEquip(int x, int y, std::shared_ptr<Equipment> item)
{
    ...
    switch (item->GetType())
    {
        case Equipment::EquipType::ARMOUR:
            if (item->GetArmourProperties().armourType == ArmourProperties::ArmourType::HEADGEAR && m_eEquipSlots != Stats::EquipSlots::HEADGEAR || item->GetArmourProperties().
                armourType == ArmourProperties::ArmourType::CHEST_BODY && m_eEquipSlots != Stats::EquipSlots::CHEST_BODY || item->GetArmourProperties().armourType ==
                ArmourProperties::ArmourType::FOOTWEAR && m_eEquipSlots != Stats::EquipSlots::FOOTWEAR)
                return;
            break;
        ...
        case Equipment::EquipType::ACCESSORY:
            if (m_eEquipSlots != Stats::EquipSlots::ACCESSORY)
                return;
            break;
        case Equipment::EquipType::NO_TYPE: CRPG_LOG("THERE SHOULD NEVER BE NO TYPE FOR EQUIPMENT");
            return;
    }
    ...
    if (index < m_EquipmentSelector.GetData().size())
    {
        ...
        m_Console.Write(m_CenterScreenW - (item_desc.size() / 2), 32, item_desc);
        // TODO: Center the description based on the half size and center - Item Description
    }
}
...
void EquipmentMenuState::RenderEquipSlots(int x, int y, const std::wstring &item)
{
    ...
    // X = 30, Y = 15 - X moves left and right
