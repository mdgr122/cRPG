...
std::shared_ptr<Equipment> EquipmentLoader::CreateObjectFromFile(const std::string& objName)
{
	...
	std::shared_ptr<Equipment> newEquipment{ nullptr }; // Initialized to nullptr
	...
	while (pItem) // While the item is not nullptr
	{
		...
		if (name != objName)
		{
			pItem = pItem->NextSiblingElement(m_bWeaponLoader ? "Weapon" : "Armour"); // Checks if we're loading weapon, if true then next sibling is a weapon, if not then next sibling is armour
			continue;
		}
		...
		std::wstring desc = CharToWide(pDesc->GetText());	// Get the description as wide string
		...
		int buy_price = atoi(pBuyPrice->GetText());	// Get the description as wide string
		...
		int sell_price = atoi(pSellPrice->GetText());	// Get the description as wide string
...
ArmourProperties EquipmentLoader::CreateArmourProperties(tinyxml2::XMLElement* xmlElement)
{
	...
	if (!pArmourProps)
		return ArmourProperties(); // IF no weapon properties, send back an empty/default WeaponProperties object

	XMLElement* pDefensePower = pArmourProps->FirstChildElement("DefensePower");
	int defensePower = pDefensePower ? atoi(pDefensePower->GetText()) : 0; // Checking to see if nullptr, if false then we send back the attack power, if true then we just send back 0.

	// Get the weapon type
	XMLElement* pArmourType = pArmourProps->FirstChildElement("ArmourType");
	const std::string armour_type_str = pArmourType ? std::string{ pArmourType->GetText() } : ""; // Checking to see if nullptr, if false then we construct a string from the weapon text, if true then send back an empty string.
...
StatModifier EquipmentLoader::CreateStatModifier(tinyxml2::XMLElement* xmlElement)
{
	...
	if (!pStatModifier)
		return StatModifier(); // IF no weapon properties, send back an empty/default WeaponProperties object

	XMLElement* pModifierValue = pStatModifier->FirstChildElement("ModValue");
	int modifierValue = pModifierValue ? atoi(pModifierValue->GetText()) : 0; // Checking to see if nullptr, if false then we send back the attack power, if true then we just send back 0.

	// Get the modifier type
	XMLElement* pModifierType = pStatModifier->FirstChildElement("ModType");
	const std::string modifier_type_str = pModifierType ? std::string{ pModifierType->GetText() } : ""; // Checking to see if nullptr, if false then we construct a string from the weapon text, if true then send back an empty string.
	...
	const std::string elemental_type_str = pElementalType ? std::string{ pElementalType->GetText() } : ""; // Checking to see if nullptr, if false then we construct a string from the weapon text, if true then send back an empty string.
