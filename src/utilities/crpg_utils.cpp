...
std::string WideToStr(const std::wstring &wstr)
{
    ...
    int size_in_bytes = WideCharToMultiByte(CP_UTF8, 0, &wstr[0], static_cast<int>(wstr.size()), NULL, 0, NULL, NULL);
    std::string strTo(size_in_bytes, 0);
    WideCharToMultiByte(CP_UTF8, 0, &wstr[0], static_cast<int>(wstr.size()), &strTo[0], size_in_bytes, NULL, NULL);
    return strTo;
}
