...
private:
	...
	void OnEquipSelect(int index, std::vector<std::shared_ptr<Equipment>> data); // Vector of shared pointers of items -- Wil include the inventory of items
	void OnSlotSelect(int index, std::vector<std::wstring> data);
	void RenderEquip(int x, int y, std::shared_ptr<Equipment> item); // draw item checks to see if wstring, if not then we need to override.
