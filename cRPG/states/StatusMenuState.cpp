...
void StatusMenuState::DrawPlayerInfo()
{
	...
	i = 1;
	const auto& stat_list = m_Player.GetStats().GetStatList(); //unordered map of wstrings to int value
	...
	{
		const auto& mod_value = m_Player.GetStats().GetModifier(stat); // Get modifier at for the specific stat we're looping through
		m_Console.Write(STAT_LABEL_X_POS, STAT_LABEL_START_Y_POS + i, stat);
		m_Console.Write(STAT_VAL_X_POS, STAT_LABEL_START_Y_POS + i, std::to_wstring(value)); // because this is a sharedptr, it could be a nullptr, so we need to check that by checking if it's equipped
		i++;
	}
}