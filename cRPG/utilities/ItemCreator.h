...
public:
	...
	template<class ...Args>
	static std::shared_ptr<Equipment> CreateEquipment(Equipment::EquipType type, WeaponProperties weapon_properties, ArmourProperties armour_properties, StatModifier stat_modifier, Args&& ...args)
	{
		switch (type)
		{
...
private:
	...
	{
		return std::make_shared<T>(std::forward<Args>(args) ...); // If the item we're trying to create is not constructible from the variable arguments we pass it, then this function won't be called by the compiler
	}
