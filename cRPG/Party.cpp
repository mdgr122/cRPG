...
bool Party::AddMember(std::shared_ptr<Player> newMember)
{
	...
	m_PartyMembers.push_back(std::move(newMember)); // Move member into the m_PartyMembers vector, and because it is a shared pointer, we can copy or move them, so we're going to move the new member in.
	return true;
